...
export default function UploadExpensesPage() {
  ...
  useEffect(() => {
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        // Reverse mapping for the parser (header -> field)
        const finalMapping: Record<string, string> = {}
        if (newMapping.date) finalMapping[newMapping.date] = "date"
        if (newMapping.description) finalMapping[newMapping.description] = "description"
        if (newMapping.amount) finalMapping[newMapping.amount] = "amount"
        // Actually we need to store what field each header maps to
        // The parser expects columnMapping[header] = field
      }
  ...
  const handleUpload = async () => {
    ...
    // Create mapping expected by parser: Record<header, field>
    const finalMapping: Record<string, string> = {}
